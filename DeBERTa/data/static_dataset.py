...
class StaticDataset(Dataset):
  def __init__(self, corpus, feature_fn, dataset_size=None, shuffle=False,supplementary=None, **kwargs):
    self.corpus = corpus
    self.supp = supplementary
  ...
  def __getitem__(self, idx):
    ...
    supp = self.supp[example_idx]
    if self.supp is None:
      return self.feature_fn(example, rng, ext_params = ext_params)
    else:
      return self.feature_fn((example,supp), rng, ext_params=ext_params)
